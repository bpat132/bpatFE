...
.if FE8 == 1
	...
	bne		IsEffective
.endif
@check if enemy has crit boost
mov     r1,#0x4 @class data pointer in character unit
ldr	r0,[r4,r1] @load the unit's class data pointer
mov     r1,#0x28 @class attributes
ldrh	r0,[r0,r1] @load the class's attributes
mov     r1,#0x40 @crit boost
and     r0,r1
cmp     r0,#0
bne     IsCritty
