	...
	@       +017 | exp   : 7
	@   +0F | u8[8] wexp
	@   +17 | u8[6] supports (REDUCED FROM u8[7])
	@   +1D | u8 level)
	@   +1E | u16[5] items
	@   +28 | u32 state
	@   +2C | (end)
...
PackGameSaveUnit.lop_supports:
	...
	@ LEVEL (new)
	ldrb r2, [r1, #0x08] @ r3 = Unit->level
	strb r2, [r0, #0x1D] @ GameSaveUnit->level = Unit->level
...
UnpackGameSaveUnit.yes_y:
	...
	@mov  r3, #0x1F
	@and  r3, r2 @ r3 = GameSaveUnit->level
...
UnpackGameSaveUnit.lop_supports:
	...
	ldrb r2, [r5, #0x1D] @ r3 = Unit->level
	strb r2, [r4, #0x08] @ GameSaveUnit->level = Unit->level
